...
public final class Limelight {
    ...
    public static boolean isPoseValid(Pose2d botPose, Pose2d visionBot) {
        return visionBot.getTranslation().getDistance(botPose.getTranslation()) < LimelightConstants.MAX_ERROR_VISION_POSE;
    }
    ...
    public static boolean atSpeaker() {
        if (hasTargets()) {
            if (DriverStation.getAlliance().get() == Alliance.Red) {
                return MathUtil.compareDouble(getApriltagId(), LimelightConstants.ID_SPEAKER_RED[0]) || MathUtil.compareDouble(getApriltagId(), LimelightConstants.ID_SPEAKER_RED[1]);
            } else {
                return MathUtil.compareDouble(getApriltagId(), LimelightConstants.ID_SPEAKER_BLUE[0]) || MathUtil.compareDouble(getApriltagId(), LimelightConstants.ID_SPEAKER_BLUE[1]);
            }
        }
        return false;
    }
    ...
    public static boolean atStage() {
        if (hasTargets()) {
            if (DriverStation.getAlliance().get() == Alliance.Red) {
                return MathUtil.compareDouble(getApriltagId(), LimelightConstants.ID_STAGE_RED[0]) || MathUtil.compareDouble(getApriltagId(), LimelightConstants.ID_STAGE_RED[1]) || 
                MathUtil.compareDouble(getApriltagId(), LimelightConstants.ID_STAGE_RED[2]);
            } else {
                return MathUtil.compareDouble(getApriltagId(), LimelightConstants.ID_STAGE_BLUE[0]) || MathUtil.compareDouble(getApriltagId(), LimelightConstants.ID_STAGE_BLUE[1]) ||
